  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      if (type === 'custom') {
        ...
        if (selectedCategory) {
          // 调用新增后端接口 /api/custom ，后端会根据 query（可能是站点 key 或关键词）返回统一的结果数组
          const res = await fetch(
            `/api/custom?query=${encodeURIComponent(selectedCategory.query)}&pageStart=0`
          );
          const json = await res.json();
          data = {
            code: 200,
            message: json.message || 'success',
            list: json.list || json.results || (Array.isArray(json) ? json : []),
          } as DoubanResult;
        } else {
      ...
      const fetchMoreData = async () => {
        ...
        try {
          ...
          if (type === 'custom') {
            ...
            if (selectedCategory) {
              // 使用 /api/custom 分页（pageStart 以 25 为步长）
              const res = await fetch(
                `/api/custom?query=${encodeURIComponent(selectedCategory.query)}&pageStart=${currentPage * 25}`
              );
              const json = await res.json();
              data = {
                code: json.code || 200,
